"""Multi-level in-memory cache for the TZP RAG Interceptor.

Levels:
  L1  trex_id  →  raw TZPPayload
  L2  trex_id  →  decompressed fallback text
  L3  cache_key →  chunk list
  L4  cache_key →  (FAISS index, chunk list)

All levels share a single LRU eviction policy keyed by
(level, key).  Structure is designed to allow a future DiskCache
backend without changing the public API.
"""

from __future__ import annotations

import hashlib
import threading
from collections import OrderedDict
from typing import Any, Protocol

from .models import TZPPayload


class CacheBackend(Protocol):
    """Abstract backend — implement for disk / Redis etc."""

    def get(self, level: str, key: str) -> Any | None: ...
    def set(self, level: str, key: str, value: Any) -> None: ...
    def delete(self, level: str, key: str) -> None: ...
    def clear(self) -> None: ...


class InterceptorCache:
    """Thread-safe LRU in-memory cache for the interceptor pipeline."""

    def __init__(self, max_entries: int = 128) -> None:
        self._max = max_entries
        self._store: OrderedDict[tuple[str, str], Any] = OrderedDict()
        self._lock = threading.Lock()
        self._hits = 0
        self._misses = 0

    # -- internal helpers ---------------------------------------------------

    def _get(self, level: str, key: str) -> Any | None:
        with self._lock:
            compound = (level, key)
            val = self._store.get(compound)
            if val is not None:
                self._store.move_to_end(compound)
                self._hits += 1
                return val
            self._misses += 1
            return None

    def _set(self, level: str, key: str, value: Any) -> None:
        with self._lock:
            compound = (level, key)
            if compound in self._store:
                self._store.move_to_end(compound)
            self._store[compound] = value
            while len(self._store) > self._max:
                self._store.popitem(last=False)

    # -- L1: raw payload ----------------------------------------------------

    def get_payload(self, trex_id: str) -> TZPPayload | None:
        return self._get("payload", trex_id)

    def set_payload(self, trex_id: str, payload: TZPPayload) -> None:
        self._set("payload", trex_id, payload)

    # -- L2: decompressed text ---------------------------------------------

    def get_text(self, trex_id: str) -> str | None:
        return self._get("text", trex_id)

    def set_text(self, trex_id: str, text: str) -> None:
        self._set("text", trex_id, text)

    # -- L3: chunk list -----------------------------------------------------

    def get_chunks(self, key: str) -> list[str] | None:
        return self._get("chunks", key)

    def set_chunks(self, key: str, chunks: list[str]) -> None:
        self._set("chunks", key, chunks)

    # -- L4: FAISS index + chunk list ---------------------------------------

    def get_index(self, key: str) -> tuple[Any, list[str]] | None:
        return self._get("index", key)

    def set_index(self, key: str, index: Any, chunks: list[str]) -> None:
        self._set("index", key, (index, chunks))

    # -- key builder --------------------------------------------------------

    @staticmethod
    def build_cache_key(
        trex_id: str,
        embedding_model: str,
        chunk_size: int,
        chunk_overlap: int,
    ) -> str:
        raw = f"{trex_id}:{embedding_model}:{chunk_size}:{chunk_overlap}"
        return hashlib.sha256(raw.encode()).hexdigest()[:24]

    # -- housekeeping -------------------------------------------------------

    def clear(self) -> None:
        with self._lock:
            self._store.clear()
            self._hits = 0
            self._misses = 0

    def stats(self) -> dict[str, int]:
        with self._lock:
            return {
                "entries": len(self._store),
                "max_entries": self._max,
                "hits": self._hits,
                "misses": self._misses,
            }
